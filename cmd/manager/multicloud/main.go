...
func init() {
	...
	utilruntime.Must(multicloudv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "multicloud",
		LeaderElectionID:   "40558063.oci.multicloud",
		SkipFIPS:           true,
	}, managerservices.ForGroup("multicloud")); err != nil {
		os.Exit(1)
	}
}
