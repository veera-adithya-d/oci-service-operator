...
func init() {
	...
	utilruntime.Must(jmsutilsv1beta1.AddToScheme(scheme))
}

func main() {
	if err := manager.Run(manager.Options{
		Scheme:             scheme,
		MetricsServiceName: "jmsutils",
		LeaderElectionID:   "40558063.oci.jmsutils",
		SkipFIPS:           true,
	}, managerservices.ForGroup("jmsutils")); err != nil {
		os.Exit(1)
	}
}
